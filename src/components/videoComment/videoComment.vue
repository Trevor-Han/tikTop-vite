<template>
    ...
    <ul class="comment_ul">
      <li v-if="state.videoComment">
        <transition-group appear>
          <li class="comment_li" v-for="(item,index) in state.videoComment" :key="index"
              ...
              <div class="comment_author_top">
                ...
                <div class="icon-shoucang1_box" @click.stop="commentLove(item,index,-1)">
                  <div class="icon_right_change"
                    ...
                    <van-icon name="like" class="icon-shoucang1"></van-icon>
                  </div>
      ...
      <li class="no_message" v-if="state.videoComment.length === 0">
  ...
  <div class="comment_input_box">
    <input :placeholder="state.commentPlaceholder" class="comment_input" v-model="state.comment_text" ref="content"
           @keyup.enter="checkComment"/>
    <!--</form>-->
    <div class="comment_input_right" @click="checkComment">
      <van-icon name="smile-comment-o" class="comment_i" :class="state.canSend?'comment_i_active':''"></van-icon>
    </div>
  </div>
</template>
...
<script setup>
  ...
  function commentLove(item, index, index2) {
    let love_comment = item.love_comment,
  ...
  function checkComment() {
    if (state.comment_text === '') {
      ...
      let comment_id = 0, p_id = 'N_1', p_user_id = '', content = state.comment_text;
  ...
  function sendComment(comment_id, p_id, p_user_id, content) {
    ...
    setTimeout(() =>{
      ...
      if (state.replayUserData === ''){
        ...
        let index = state.replayUserData.index;
        let index2 = state.replayUserData.index2;
